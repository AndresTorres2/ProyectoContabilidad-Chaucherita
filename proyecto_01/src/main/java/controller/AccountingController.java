...
public class AccountingController extends HttpServlet {
	...
	public void viewAccount(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
	    ...
	    List<Movement> outcomeMovements =  outcomeDAO.getMovementsByAccount(account, startDate, endDate);
		List<Movement> incomeMovements = incomeDAO.getMovementsByAccount(account, startDate, endDate);
		List<Movement> transferenceMovements = transferenceDAO.getMovementsByAccount(account, startDate, endDate);
	    
	    
		List<Movement> movements = new ArrayList<>();
		movements.addAll(outcomeMovements);
		movements.addAll(incomeMovements);
		movements.addAll(transferenceMovements);

	    List<MovementDTO> movementsDTO= movementDAO.getAllMovementsDTO(movements);
	...
	public void outcome(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
	    ...
	    try{
		    ...
		    outcomeDAO.createOutcome(newOutcome);
	        //actualiza la cuenta
		    if("showdashboard".equals(origen)) {
